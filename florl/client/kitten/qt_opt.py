...
class QTOptProx(QTOpt):
    ...
    def _critic_update(
        ...
        proximal_mu: float = 0,
        global_knowledge: QTOptKnowledge | None = None,
    ):
        # ===== Kitten Code =====
        ...
        if global_knowledge is not None:
            for local_weights, global_weights in zip(
                self._critic_1.net.parameters(),
                global_knowledge.critic_1.net.parameters(),
            ):
                proximal_term += torch.square((local_weights - global_weights).norm(2))
            for local_weights, global_weights in zip(
                self._critic_2.net.parameters(),
                global_knowledge.critic_2.net.parameters(),
            ):
                proximal_term += torch.square((local_weights - global_weights).norm(2))
    ...
    def update(
        ...
        if step % self._update_frequency == 0:
            self.loss_critic_value = self._critic_update(
                batch, aux, proximal_mu, global_knowledge
            )
